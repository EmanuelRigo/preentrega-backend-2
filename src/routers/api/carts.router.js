...
cartApiRouter.get("/", async (req, res) => {
  const process = await controller.get();
  res.status(200).send({ error: null, data: process });
});

cartApiRouter.get("/:cid", async (req, res) => {
...
cartApiRouter.put("/:cid/products/:pid", async (req, res) => {
...
cartApiRouter.delete("/:cid/products/:pid", async (req, res) => {
  const { cid, pid } = req.params;
...
cartApiRouter.delete("/:cid", async (req, res) => {
